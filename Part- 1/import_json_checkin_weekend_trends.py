...
'''
for row in c.execute('select c_day from yl_checkin inner join yl_business_head on yl_checkin.business_id=yl_business_head.business_id WHERE yl_business_head.city="Las Vegas" or yl_business_head.city="Phoenix" LIMIT 10000'):
...
plt.xticks(range(len(dictUSA)), dictUSA.keys(), align="center")
plt.xlim([0,24])

plt.xlabel('Hour')
plt.ylabel('Amount of Check-ins')
plt.savefig('checkindays_USA.png',dpi=300,bbox_inches='tight')
...
for row in c.execute('select c_day from yl_checkin inner join yl_business_head on yl_checkin.business_id=yl_business_head.business_id WHERE yl_business_head.city="Edinburgh" LIMIT 10000'):
    wordsUK.append(row)
    dictUK.update(row)
...
plt.xlabel('Hour')
plt.ylabel('Amount of Check-ins')
plt.savefig('checkindays_UK.png',dpi=300,bbox_inches='tight')
...
for row in c.execute('select c_day from yl_checkin inner join yl_business_head on yl_checkin.business_id=yl_business_head.business_id WHERE yl_business_head.city="Stuttgart" or yl_business_head.city="Ditzingen" or yl_business_head.city="Hemmingen" LIMIT 10000'):
    wordsGER.append(row)
    dictGER.update(row)
...
plt.xlabel('Hour')
plt.ylabel('Amount of Check-ins')
plt.savefig('checkindays_GER.png',dpi=300,bbox_inches='tight')
...
for row in c.execute('select c_day from yl_checkin inner join yl_business_head on yl_checkin.business_id=yl_business_head.business_id WHERE yl_business_head.city="Toronto" or yl_business_head.city="Sainte-Anne-de-Bellevue" LIMIT 10000'):
    wordsCAN.append(row)
    dictCAN.update(row)
...
plt.xlabel('Hour')
plt.ylabel('Amount of Check-ins')
plt.savefig('checkindays_CAN.png',dpi=300,bbox_inches='tight')
